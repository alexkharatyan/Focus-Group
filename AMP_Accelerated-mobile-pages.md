...
## Cache updates
The AMP Cache uses a [one-behind caching model](https://developers.google.com/amp/cache/overview#google-amp-cache-updates) which can be controlled by the max-age [cache](https://developer.mozilla.org/de/docs/Web/HTTP/Headers/Cache-Control) directive. You can test the behavior with this sample page, which will prints the current date at serving time:
